...
$wp_plugin_url = plugins_url() . '/eu-cookiewet-plugin';

add_action('admin_menu', 'eu_cookiewet_plugin_menu');
function eu_cookiewet_plugin_menu() {
	add_options_page('EU Cookiewet', 'EU Cookiewet', 'manage_options', 'eu_cookiewet_plugin', 'eu_cookiewet_opties_pagina');
}
...
add_action('admin_init', 'eu_cookie_admin_init');
function eu_cookie_admin_init(){
	register_setting( 'eu_cookiewet_opties', 'eu_cookiewet_opties', 'eu_cookiewet_opties_validate' );
	add_settings_section('eu_cookie_main', '', 'eu_cookie_section_text', 'eu_cookie', 'eu_cookie_main' );
	add_settings_field('eu_cookie_text', 'Notificatie', 'eu_cookie_text_settings', 'eu_cookie', 'eu_cookie_main' );
	add_settings_field('eu_cookie_accept', 'Knop voor acceptatie', 'eu_cookie_accept_settings', 'eu_cookie', 'eu_cookie_main' );
	add_settings_field('eu_cookie_more', 'Meer informatie link', 'eu_cookie_more_settings', 'eu_cookie', 'eu_cookie_main' );	
}

function eu_cookie_section_text() {
...
function eu_cookie_text_settings() {
	$options = get_option( 'eu_cookiewet_opties' );
	...
	echo "<input id='eu_cookie_text_settings' name='eu_cookiewet_opties[eu_cookie_text_settings]' size='50' type='text' value='{$value}' />";
}
function eu_cookie_accept_settings() {
	$options = get_option('eu_cookiewet_opties');
...
function eu_cookie_more_settings() {
	$options = get_option('eu_cookiewet_opties');
...
function eu_cookiewet_opties_validate($input) {
	$options = get_option( 'eu_cookiewet_opties' );
...
function eu_cookie_jquery() {
    wp_enqueue_script( 'jquery' );
}     
add_action('wp_enqueue_scripts', 'eu_cookie_jquery');

function eu_add_cookie_css() {
...
add_action ( 'wp_head', 'eu_add_cookie_css' );

function eu_add_cookie_bar() {
	if ( !isset ( $_COOKIE["catAccCookies"] ) ) {
		$options = get_option('eu_cookiewet_opties');
...
add_action ( 'wp_footer', 'eu_add_cookie_bar', 1000 );
